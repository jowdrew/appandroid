    ...
    return ListView(
      ...
      children: [
        ...
        _SectionHeader(
          ...
          subtitle: 'Comparte lo planificado vs lo gastado',
        ),
        const SizedBox(height: 12),
...
class ChartTypeSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SegmentedButton<ChartType>(
      segments: const [
        ButtonSegment(value: ChartType.bar, label: Text('Barras')),
        ButtonSegment(value: ChartType.pie, label: Text('Dona')),
        ButtonSegment(value: ChartType.line, label: Text('Línea')),
        ButtonSegment(value: ChartType.area, label: Text('Área')),
      ],
      selected: {selected},
...
class _ChartSwitcher extends StatelessWidget {
  const _ChartSwitcher({required this.type, required this.data});
  ...
  @override
  Widget build(BuildContext context) {
    // Render a single chart based on the selected type.
    switch (type) {
      case ChartType.bar:
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: CustomPaint(
          size: const Size(double.infinity, 160),
          painter: _LineChartPainter(data, fillArea: false),
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: CustomPaint(
          size: const Size(double.infinity, 160),
          painter: _LineChartPainter(data, fillArea: true),
        ),
      ),
    );
  }
...
class _PieChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // Simple donut chart using the same category totals as the bar chart.
    ...
    for (final item in data) {
      final sweep = (item.total / total) * 6.28318;
      final paint = Paint()
...
class _LineChartPainter extends CustomPainter {
  _LineChartPainter(this.data, {required this.fillArea});
  ...
  @override
  void paint(Canvas canvas, Size size) {
    // Simple line/area chart using category totals as sequential points.
    final totals = data.map((item) => item.total).toList();
    ...
    final step = totals.length == 1 ? 0.0 : width / (totals.length - 1);
    ...
    for (var i = 0; i < totals.length; i++) {
      ...
      final y = padding + height - (totals[i] / maxValue) * height;
      if (i == 0) {
        linePath.moveTo(x, y);
    ...
    areaPath.lineTo(padding + width, padding + height);
    areaPath.close();

    if (fillArea) {
      final gradient = LinearGradient(
        colors: [
          data.first.category.color.withOpacity(0.35),
          data.first.category.color.withOpacity(0.05),
        ],
    ...
    final linePaint = Paint()
      ..color = data.first.category.color
      ..style = PaintingStyle.stroke
      ..strokeWidth = 3
      ..strokeCap = StrokeCap.round;
...
class _BudgetScreenState extends State<BudgetScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final currency = currencyFormatter();
    final hasAnyBudget = store.budgets.isNotEmpty;
    ...
    return ListView(
      ...
      children: [
        ...
        if (!hasAnyBudget)
          Text(
            'Define un presupuesto por categoría.',
            style: Theme.of(context).textTheme.bodySmall,
          ),
        const SizedBox(height: 12),
        ...filteredCategories.map((category) {
          ...
          final spent =
              store.spentForCategoryInMonth(_selectedMonth, category);
          final ratio = budget == null || budget <= 0 ? 0.0 : spent / budget;
          final progress = ratio.clamp(0, 1);
          final percentage =
              budget == null || budget <= 0 ? 0 : (ratio * 100).round();
                ...
                children: [
                  ...
                  if (currentAmount != null)
                    Expanded(
                      child: TextButton(
                        onPressed: () {
                          ExpensesScope.of(context).deleteBudget(
                            _selectedMonth,
                            category,
                          );
...
class ExpensesStore extends ChangeNotifier {
  ...
  double totalExpensesForMonth(DateTime month) {
    return totalForType(
  ...
  List<Budget> budgetsForMonth(DateTime month) {
    ...
    return _budgets
        .where((budget) => budget.month == normalized)
        .toList();
  }
  ...
  double? budgetFor(DateTime month, MovementCategory category) {
    ...
    for (final budget in _budgets) {
      if (budget.month == normalized && budget.category.id == category.id) {
  ...
  void upsertBudget(
      DateTime month, MovementCategory category, double amount) {
    final normalized = normalizeMonth(month);
    final index = _budgets.indexWhere(
        (budget) => budget.month == normalized && budget.category.id == category.id);
    if (index == -1) {
      _budgets.add(
        Budget(
          id: DateTime.now().millisecondsSinceEpoch.toString(),
  ...
  double spentForCategoryInMonth(
      DateTime month, MovementCategory category) {
    return _movements
        .where((movement) =>
            movement.type == MovementType.expense &&
            movement.date.year == month.year &&
            movement.date.month == month.month &&
  ...
  BudgetSummary budgetSummaryForMonth(DateTime month) {
    final totalBudget = budgetsForMonth(month)
        .fold(0, (sum, item) => sum + item.amount);
